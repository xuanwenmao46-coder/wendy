...
const FRAMES_DIR = path.join(__dirname, 'frames_cf');
const OUT_MP4    = path.join(__dirname, 'control-failure.mp4');
const FPS        = 30;
const DURATION   = 80; // seconds — slightly padded for safety
const W          = 1280;
const H          = 720;
...
(async () => {
  ...
  await page.goto(
    'file://' + path.join(__dirname, 'control-failure.html'),
  ...
  for (let f = 0; f < totalFrames; f++) {
    ...
    // Extra settle for word erosion section (CH05) and SAFE grid infection (CH04)
    const settle = (t >= 28 && t <= 48) ? 18 : 8;
