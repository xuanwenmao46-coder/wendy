...
const FRAMES_DIR = path.join(__dirname, 'frames_v2');
const OUT_MP4    = path.join(__dirname, 'fire-is-hungry-v2.mp4');
const FPS        = 30;
const DURATION   = 74; // seconds
const W          = 1280;
const H          = 720;
...
(async () => {
  ...
  await page.goto(
    'file://' + path.join(__dirname, 'fire-is-hungry-v2-record.html'),
  ...
  for (let f = 0; f < totalFrames; f++) {
    ...
    // Use CDP directly to avoid protocolTimeout on evaluate
    await page.evaluate((time) => {
      window.tl.seek(time);
    }, t);

    // Longer settle for frames with call() callbacks (word erosion)
    const settle = (t >= 30 && t <= 43) ? 18 : 8;
    ...
    if (f % 150 === 0) {
      process.stdout.write(`  ${f}/${totalFrames} (${t.toFixed(1)}s)\n`);
    }
  }
